...
class Checkout extends Component
{
    ...
    public $listeners = ['productSelected', 'discountModalRefresh', 'cartUpdated'];
    ...
    public function render() {
        ...
        // Ensure setiap cart item punya key di semua arrays
        foreach ($cart_items as $item) {
            if (!isset($this->discount_type[$item->id])) {
                $this->discount_type[$item->id] = 'fixed';
            }
            if (!isset($this->item_discount[$item->id])) {
                $this->item_discount[$item->id] = 0;
            }
            if (!isset($this->quantity[$item->id])) {
                $this->quantity[$item->id] = $item->qty;
            }
            if (!isset($this->check_quantity[$item->id])) {
                $this->check_quantity[$item->id] = false;
            }
        }
    ...
    public function cartUpdated() {
        // This method is called when cart is updated via barcode scanner
