...
use Modules\Product\Entities\GroupCategory;
use Modules\Product\DataTables\ProductGroupCategoriesDataTable;

class GroupCategoriesController extends Controller
{

    public function index(ProductGroupCategoriesDataTable $dataTable) {
        abort_if(Gate::denies('access_product_categories'), 403);

        return $dataTable->render('product::group-categories.index');
    }
    ...
    public function update(Request $request, $id) {
        ...
        GroupCategory::findOrFail($id)->update([
            'category_code' => $request->category_code,
            'category_name' => $request->category_name,
        ]);
