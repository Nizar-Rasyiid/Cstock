...
Route::group(['middleware' => 'auth'], function() {
    ...
    Route::get('/purchase-returns/pdf/{id}', function ($id) {
        $data['purchase_return'] = \Modules\PurchasesReturn\Entities\PurchaseReturn::findOrFail($id);
        $data['supplier'] = \Modules\People\Entities\Supplier::findOrFail($data['purchase_return']->supplier_id);
        $data['kop'] = \App\Models\Kop::where('company_id', Auth::user()->company_id)->first(); 

        return view('purchasesreturn::print', $data);
