...
class AdjustmentController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        DB::transaction(function () use ($request) {
            ...
            foreach ($request->product_ids as $key => $id) {
                ...
                } elseif ($request->types[$key] == 'sub' || $request->types[$key] == 'etalase') {
                    $product->update([
                        'product_quantity' => $product->product_quantity - $request->quantities[$key]
                    ]);
                }
    ...
    public function edit(Adjustment $adjustment) {
        ...
        return view('adjustment::edit', compact('adjustment', 'categories'));
    }
    ...
    public function update(Request $request, Adjustment $adjustment) {
        ...
        DB::transaction(function () use ($request, $adjustment) {
            ...
            foreach ($adjustment->adjustedProducts as $adjustedProduct) {
                ...
                } elseif ($adjustedProduct->type == 'sub' || $adjustedProduct->type == 'etalase') {
                    $product->update([
                        'product_quantity' => $product->product_quantity + $adjustedProduct->quantity
                    ]);
                }
