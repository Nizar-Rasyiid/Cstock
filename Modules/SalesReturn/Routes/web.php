...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::get('/sale-returns/pdf/{id}', function ($id) {
        $data['sale_return'] = \Modules\SalesReturn\Entities\SaleReturn::findOrFail($id);
        $data['customer'] = \Modules\People\Entities\Customer::findOrFail($data['sale_return']->customer_id);
        $data['kop'] = \App\Models\Kop::where('company_id', Auth::user()->company_id)->first();

        return view('salesreturn::print', $data);
